...
int interface()
{
	...
	int matrday[7][8];
	FILE *data = 0;
	int x0=0,y0=0,flag=0,pole = 0,flagtext = 0,namdertask = 1,ntask,ntaskp,mtask = 0,i = 1, k,help, help1, m, flag3 = 0, flag4 = 0, flag5 = 0, helpi = 0, hpp = 0;
	float x, y, x1, y1;
	RenderWindow window(VideoMode(1300, 700), "To-do");
	nowdate(&nowday, &nowMonth);
	...
	sf::Time t1 = sf::milliseconds(1);
	sf::Time t2 = sf::milliseconds(50);
	sf::Time t3 = sf::milliseconds(100);

	Image basket;
	basket.loadFromFile("../image/basket.png");
	Texture baskettexture;
	baskettexture.loadFromImage(basket);
	Sprite basketsprite;
	basketsprite.setTexture(baskettexture);

	Image exclaim;
	exclaim.loadFromFile("../image/exclaim.png");
	Texture exclaimtexture;
	exclaimtexture.loadFromImage(exclaim);
	Sprite exclaimsprite;
	exclaimsprite.setTexture(exclaimtexture);

	Image notexclaim;
	notexclaim.loadFromFile("../image/notexclaim.png");
	Texture notexclaimtexture;
	notexclaimtexture.loadFromImage(notexclaim);
	Sprite notexclaimsprite;
	notexclaimsprite.setTexture(notexclaimtexture);
		...
		if (!pole) {
					...
					if (flag == 6) {
						...
						if (NotesDay[namdertask].n_text[help].t_metadata != 0)
							if (help < ntaskp) {
								if (NotesDay[namdertask].n_text[help].t_metadata == 2) {
									NotesDay[namdertask].n_text[help].t_metadata = 1;
								} else if ((NotesDay[namdertask].n_text[help].t_metadata == 1)|| (NotesDay[namdertask].n_text[help].t_metadata == 3)) {
									NotesDay[namdertask].n_text[help].t_metadata = 2;
								}
							}
					}
					if (flag == 10) {
						...
						if (help < ntask)
							if (NotesDay[help].n_text[0].t_metadata != 0) {
								...
								} else if ((NotesDay[help].n_text[0].t_metadata == 1) || (NotesDay[help].n_text[0].t_metadata == 2)) {
			...
			if (flag == 7) {
				...
				if (ntask < 10) {
					...
					NotesDay[ntask-1].n_data.tm_mon = nowMonth + 1;
					NotesDay[ntask-1].n_data.tm_year = year;
					mtask++;
					flag5 = 0;
					flag4 = 0;
					flag3 = 1;
